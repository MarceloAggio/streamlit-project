...
class AdvancedRecurrenceAnalyzer:
  ...
  def _analyze_pattern_maturity(self, df, intervals, render=True):
    ...
    if render:
      ...
      fig.update_layout(title="Evolução da Variabilidade (CV Adaptativo)", xaxis_title="Período", yaxis_title="CV", height=300)
  ...
  def _final_classification(self, results, df, intervals):
    ...
    with col1:
      ...
      st.markdown("#### 📊 Breakdown dos Critérios VALIDADOS (com CV Adaptativo)")
      total_occurrences = len(df)
      period_days = (df['created_on'].max() - df['created_on'].min()).days + 1
      freq_per_week = (total_occurrences / period_days * 7) if period_days > 0 else 0
  ...
  if uploaded_file:
    ...
    if analyzer.load_data(uploaded_file):
      if analysis_mode == "🔍 Individual":
        ...
        st.subheader("📊 Análise Completa COM CV ADAPTATIVO")
        if st.sidebar.button("🚀 Executar", type="primary"):
          ...
          if df_consolidated is not None and len(df_consolidated) > 0:
            st.success(f"✅ {len(df_consolidated)} alertas processados com CV Adaptativo!")
            st.header("📊 Resumo")
            col1, col2, col3, col4 = st.columns(4)
            ...
            # Estatísticas de CV
            st.markdown("---")
            st.subheader("🎯 Estatísticas de CV Adaptativo")
